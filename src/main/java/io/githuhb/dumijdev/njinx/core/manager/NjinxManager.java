...
public class NjinxManager {
  ...
  public NjinxManager(File nginxExecutable) {
    this(nginxExecutable.getAbsolutePath());
  }
  ...
  private static boolean findNginxInProcesses(ProcessHandle process) {
    var info = process.info();

    if (info.command().isPresent()) {
      var command = info.command().get();
      var file = new File(command);
      return file.getName().contains("nginx");
    }
  ...
  public boolean isRunning() {
    return ProcessHandle.allProcesses()
        .anyMatch(NjinxManager::findNginxInProcesses);
  }
